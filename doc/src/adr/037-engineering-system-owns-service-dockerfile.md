...
- The service JAR is selected by the `docker-build` action, not hard-wired:
  the descriptor may declare `build.artifact.path`; otherwise the caller uses
  `provider/<service>-azure/target/*-spring-boot.jar` and the action falls back
  to unambiguous Azure Spring Boot JAR discovery. The JAR is always built from
...
- A new or Dockerfile-less service builds an image with zero per-service
  Docker work: service identity, Maven profiles and any exceptional JAR path
  are reviewed in the descriptor, and the canonical Dockerfile arrives via
  sync.
...
- A service whose Azure module name deviates from the `<name>-azure`
  convention needs no configuration when discovery is unambiguous. A service
  with multiple Azure JARs declares `build.artifact.path`.
